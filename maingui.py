...
c.execute(
    "update rooms set status=0 where roomn not in (select bookings.roomno from bookings);")  # Daily necessary tasks
mydb.commit()
...
def new():
    ...
    if len(rn) == 0:
        ...
        id2 = c.fetchall()
        messagebox.showinfo("ID & Room No.",
                            "Your Booking ID is: " + str(id2[0][0]) + " and Room No is: " + str(id2[0][1]))
...
showbook = Button(mainwin, text="SHOW BOOKINGS", command=shwin, font='times 16 bold', activebackground="pink",
                  activeforeground="blue").place(x=520, y=50)
...
submitbtn = Button(leftframe, text="Submit", command=new, activebackground="pink", activeforeground="blue").place(x=200,
                                                                                                                  y=270)
...
cancelbtn = Button(leftframe, text="Submit", command=cancel, activebackground="pink", activeforeground="blue").place(
    x=150, y=500)
mainwin.mainloop()
